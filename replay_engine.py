...
PCARDS = lambda cards: "[{}]".format(
    " ".join(map(str, cards))
)  ### Changed from PCARDS = lambda cards: '[{}]'.format(' '.join(map(str, cards)))
...
class RoundState(
    ...
    def legal_actions(self):
        ...
        if continue_cost == 0:
            ...
            return (
                ...
                if bets_forbidden
                else {CheckAction, RaiseAction, FoldAction}
            )
        # continue_cost > 0
    ...
    def proceed(self, action):
        ...
        if isinstance(action, CheckAction):
            if (
                (self.street == 0 and self.button > 0)
                or self.button > 1
                or self.street == 2
                or self.street == 3
            ):  # both players acted
                ...
                if action in legal_actions:
                    if clause[0] == "R":
                        ...
                        if 0 <= card <= 2:
                            ...
                            game_log.append(
                                f"{self.name} attempted to discard invalid index {card}"
                            )
...
class Game:
    ...
    def log_round_state(self, players, round_state):
        ...
        if round_state.street == 0:
            self.preflop_bets = {
                players[0].name: STARTING_STACK - round_state.stacks[0],
                players[1].name: STARTING_STACK - round_state.stacks[1],
            }
        elif round_state.street == 4:
            self.flop_bets = {
                players[0].name: STARTING_STACK
                - round_state.stacks[0]
                - self.preflop_bets[players[0].name],
                players[1].name: STARTING_STACK
            ...
            self.turn_bets = {
                players[0].name: STARTING_STACK
                - round_state.stacks[0]
                - self.flop_bets[players[0].name]
                - self.preflop_bets[players[0].name],
                players[1].name: STARTING_STACK
                - round_state.stacks[1]
                - self.flop_bets[players[1].name]
                - self.preflop_bets[players[1].name],
            }
        ...
        if round_state.street == 0 and round_state.previous_state is None:
            ...
            and round_state.button == 1
        ) or (round_state.street == 3 and round_state.button == 0):
            board = round_state.board
            self.log.append(
                STREET_NAMES[round_state.street - 2]
                + " "
                + PCARDS(board)
            ...
            self.log.append(
                f"Current stacks: {round_state.stacks[0]}, {round_state.stacks[1]}"
            )
    ...
    def run_round(self, players):
        ...
        while not isinstance(round_state, TerminalState):
            ...
            self.log_action(
                player.name, action, bet_override, round_state.hands[active]
            )
        ...
        for i in range(len(players)):
            multiplier = (
                1
                if round_state.deltas[i] > 0
                else (0 if round_state.deltas[i] == 0 else -1)
            )
            self.ev_preflop_bets[players[i].name] += (
                multiplier * self.preflop_bets[players[i].name]
            )
            self.ev_flop_bets[players[i].name] += (
                multiplier * self.flop_bets[players[i].name]
            )
            self.ev_turn_bets[players[i].name] += (
                multiplier * self.turn_bets[players[i].name]
            )
        for player, player_message, delta in zip(
            players, self.player_messages, round_state.deltas
        ):
    ...
    def run(self):
        ...
        for player in players:
            self.log.append(
                "{} preflop bets EV: {}".format(
                    player.name, self.ev_preflop_bets[player.name]
                )
            )
            self.log.append(
                "{} flop bets EV: {}".format(
                    player.name, self.ev_flop_bets[player.name]
                )
            )
            self.log.append(
                "{} turn bets EV: {}".format(
                    player.name, self.ev_turn_bets[player.name]
                )
            )
            player.stop()
