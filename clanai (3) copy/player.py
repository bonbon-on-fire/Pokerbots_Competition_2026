...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if continue_cost > 0:
            ...
            if late_street:
                # Risk premium scales with bet size; avoid over-folding vs tiny river bets.
                risk_premium = 0.01 + 0.06 * bet_fraction
                call_threshold = min(0.97, call_threshold + min(0.08, risk_premium))
                if street == 6 and bet_fraction <= 0.25:
                    call_threshold = min(call_threshold, max(0.0, pot_odds - 0.02))
                if street == 6 and bet_fraction >= 0.45:
                    # Require very strong equity versus big river bets.
                    strong_req = 0.90 + 0.05 * min(1.0, bet_fraction)
                    if win_probability < max(strong_req, pot_odds + 0.15):
            ...
            if late_street and big_bet and win_probability < 0.88:
                return FoldAction() if FoldAction in legal_actions else CheckAction()
            # Even tighter on the river against big bets.
            if street == 6 and big_bet and win_probability < 0.92:
                return FoldAction() if FoldAction in legal_actions else CheckAction()
    ...
    def sim_mc_once(self, my_cards, board_cards, discard_idx, opp_has_three=None):
        sample = random.sample
        remaining = self.REMAINING_DECK
        new_board = board_cards.copy()
        kept_cards = my_cards.copy()
        ...
        if opp_has_three is None:
            opp_has_three = len(new_board) <= 3

        if opp_has_three:
            # Opponent still has 3 cards and will discard one onto the board.
            needed = max(0, 6 - (len(new_board) + 1))
            draw = sample(remaining, 3 + needed)
            opp_cards = draw[:3]
            opp_discard_idx = self.choose_opponent_discard_simple(opp_cards, new_board)
            opp_discard = opp_cards[opp_discard_idx]
            opp_kept_cards = [c for i, c in enumerate(opp_cards) if i != opp_discard_idx]
            future_board = draw[3 : 3 + needed]
            final_board = new_board + [opp_discard] + future_board
        else:
            # Opponent already discarded and has 2 cards.
            needed = max(0, 6 - len(new_board))
            draw = sample(remaining, 2 + needed)
            opp_kept_cards = draw[:2]
            future_board = draw[2 : 2 + needed]
            final_board = new_board + future_board

        my_hand = kept_cards + final_board
        opp_hand = opp_kept_cards + final_board
    ...
    def _calc_winning_prob(self, my_cards, board_cards, street=0):
        ...
        if street == 0:
            ...
            return self.preflop_equities.get(hand_key, 0.5)
        ...
        opp_has_three = len(board_cards) <= 3
        for _ in range(mc_iters):
            # # Get remaining deck
            ...
            increase = sim_once(
                my_cards, board_cards, discard_idx=-1, opp_has_three=opp_has_three
            )
