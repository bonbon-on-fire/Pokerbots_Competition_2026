...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if RaiseAction in legal_actions:
            # pot_odds = continue_cost / ( continue_cost + (my_contribution + opp_contribution) )
            ...
            if ev <= 0 and CheckAction in legal_actions:
                return CheckAction()
            elif ev > 0 and RaiseAction in legal_actions:
                ...
                return RaiseAction(min(round(raise_amount), max_raise))
            return FoldAction()
